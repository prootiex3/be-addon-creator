...
class AddonManager:
    def __init__(self, directory: pathlib.Path, name: str, description: str) -> None:
        self.main_directory = directory
        self.clean()
        technical_name = '_'.join(name.lower().split(' '))
        self.namespace = technical_name
        self.behaviour_path = directory.joinpath(f'{technical_name}_behaviour')
        self.resource_path = directory.joinpath(f'{technical_name}_resources')
    ...
    def __ensure_file_or_folder_exists(self, path: pathlib.Path, is_folder: bool = False):
        if not path.exists():
            if is_folder:
    ...
    def __setup_behaviour_manifest(self, resource_manifest) -> dict:
        ...
        manifest_path = self.__ensure_file_or_folder_exists(path=self.behaviour_path.joinpath('manifest.json'))
        manifest = {
    ...
    def __setup_resources_manifest(self) -> dict:
        ...
        manifest_path = self.__ensure_file_or_folder_exists(path=self.resource_path.joinpath('manifest.json'))
        manifest = {
            'format_version': FORMAT_VERSION,
            'header': {
    ...
    def write_to_lang(self, key: str, value: str):
        lang_path = self.__ensure_file_or_folder_exists(path=self.resource_path.joinpath('texts'), is_folder=True)
        english_lang_path = self.__ensure_file_or_folder_exists(path=lang_path.joinpath('en_US.lang'))
        english_lang_path.write_text(f'\n{key}={value}')

    def create_item(self, id: str, display_name: str, category: CreativeCategory = CreativeCategory.Construction, stack_size: int = 64):
        print(f'Creating item with id \'{id}\'')

        item_behaviour_path = self.__ensure_file_or_folder_exists(path=self.behaviour_path.joinpath('items'), is_folder=True)
        item_resources_path = self.__ensure_file_or_folder_exists(path=self.resource_path.joinpath('textures/items'), is_folder=True)
        ...
        item_path = self.__ensure_file_or_folder_exists(path=item_behaviour_path.joinpath(f'{id}.json'))
        item = {
